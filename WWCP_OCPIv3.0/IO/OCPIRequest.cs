                                                  ...
                                                  out var partiesAccessInfos))
                {
                    ...
                    if (partiesAccessInfos.Count() == 1)
                    {
                        ...
                        CPOIds           = RemoteParty.Roles.Where (credentialsRole => credentialsRole.Role == Role.CPO).
                                                             Select(credentialsRole => CPO_Id. Parse($"{LocalAccessInfo.Roles.First().PartyId.ToString(LocalAccessInfo.Roles.First().Role)}")).
                                                             Distinct().
                                                             ToArray();

                        EMSPIds          = RemoteParty.Roles.Where (credentialsRole => credentialsRole.Role == Role.EMSP).
                                                             Select(credentialsRole => EMSP_Id.Parse($"{LocalAccessInfo.Roles.First().PartyId.ToString(LocalAccessInfo.Roles.First().Role)}")).
                                                             Distinct().
                                                             ToArray();
