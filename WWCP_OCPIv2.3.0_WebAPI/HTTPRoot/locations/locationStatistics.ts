                    ...
                    if (evse.connectors) {
                        for (var connector of evse.connectors) {
                            ...
                            if (connector.tariff_ids) {
                                for (var tariffId of connector.tariff_ids) {

                                    // Tariffs for EVSEs in use...
                                    if (!tariffsMapActive.has(tariffId))
                                        tariffsMapActive.set(tariffId, 0);

                                    // ...or for removed EVSEs
                                    if (!tariffsMapRemoved.has(tariffId))
                                        tariffsMapRemoved.set(tariffId, 0);


                                if (evse.status === "REMOVED")
                                    tariffsMapRemoved.set(
                                        tariffId,
                                        tariffsMapRemoved.get(tariffId) + 1
                                    );

                                else
                                    tariffsMapActive.set(
                                        tariffId,
                                        tariffsMapActive.get(tariffId) + 1
                                    );
