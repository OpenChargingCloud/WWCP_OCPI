...
namespace cloud.charging.open.protocols.OCPIv2_3_0.UnitTests.CPO_PTP
{
    ...
    [TestFixture]
    public class CPO_Tests : ACPO_PTP_Tests
    {

        #region GetVersions_FromPTP_Test()

        /// <summary>
        /// The CPO will fetch the OCPI Versions from the PTP.
        /// </summary>
        [Test]
        public async Task GetVersions_FromPTP_Test()
        {

            var graphDefinedPTP = cpoAPI?.GetPTPClient(
                                      CountryCode: CountryCode.Parse("DE"),
                                      PartyId:     Party_Id.   Parse("GDF")
                                  );

            Assert.That(graphDefinedPTP,  Is.Not.Null);

            if (graphDefinedPTP is not null)
            {

                var response = await graphDefinedPTP.GetVersions();
                ...
                Assert.That(response?.Data?.First().URL,                                                         Is.EqualTo(ptpAPI?.CommonAPI.BaseAPI.OurVersionsURL + "2.3.0"));
        ...
        #region GetVersionDetails_FromPTP_Test()

        /// <summary>
        /// The CPO will fetch the OCPI VersionDetails from the PTP.
        /// </summary>
        [Test]
        public async Task GetVersionDetails_FromPTP_Test()
        {
            ...
            if (graphDefinedPTP is not null)
            {
                ...
                var response = await graphDefinedPTP.GetVersionDetails(Version_Id.Parse("2.3.0"));
